...
class Service {
  calculatePrices = async (towTypeId: ObjectId | string, distance: number, promoCode : string, userId: ObjectId | string): Promise<{amount: number, discount: number, finalAmount: number, charge: number}> => {
    const { baseFare = 0, perKM = 0, charge = 0 } = await TowType.findById(towTypeId) as ITowType;

    const amount = baseFare + perKM * distance;
    let discount = await PromoService.usePromo(promoCode, userId, amount)
